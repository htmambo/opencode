...
export const useSessionCommands = (input: {
  ...
  importSession: () => void | Promise<void>
}) => {
  const cycleFileTree = () => {
    const cycle = input.layout.fileTree.cycle()
    if (cycle === 0) {
      input.layout.fileTree.toggle()
      input.layout.fileTree.setCycle(1)
      return
    }
    if (cycle === 1) {
      input.layout.session.hidePanel()
      input.layout.fileTree.setCycle(2)
      return
    }
    input.layout.session.showPanel()
    input.layout.fileTree.setCycle(0)
  }

  const toggleReviewPanel = () => {
    if (!input.layout.session.panel()) input.layout.session.showPanel()
    input.view().reviewPanel.toggle()
  }
